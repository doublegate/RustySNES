...
- **43** in `fc.e` (`JSR (addr,X)`, emulation): the return-address push **escapes the emulation-mode
  ...
  `$01FF`. Confirmed by all four SNES-accurate references: bsnes/ares `instructionCallIndexedIndirect`
  (`pushN`), snes9x `OpFCE1` (`PushW`/`WRAP_BANK`, source comment *"JSR (a,X) is a new instruction,
  and so doesn't respect the emu-mode stack bounds"*), MesenCE `JSR_AbsIdxXInd`
  (`PushWord(PC, allowEmulationMode=false)` + `RestrictStackPointerValue()`), plus AccuracySNES
  `A3.08` on snes9x + Mesen2. SST confines the push to page 1 and is the lone outlier; RustySNES
  matches the four references on every physically-reachable state (`S.h=$01`).
