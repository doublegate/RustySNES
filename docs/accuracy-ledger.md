...
| Mid-scanline/HDMA-driven register timing (the "Air Strike Patrol BG3 scroll" case) | **Remediated** (`v0.8.0`; deepened to dot resolution in `v1.21.0`) | Originally the PPU composited each line at a single `RENDER_DOT` (dot 276) so a per-line HDMA register write became visible starting the following line. As of `v1.21.0` the **per-dot compositor** (`docs/adr/0014`, T-CA-10) is the sole renderer: the line is composited one column at a time against live registers, so a mid-line write to CGRAM/OAM/scroll/`INIDISP` reaches only the not-yet-drawn columns — validated 29/29 on undisbeliever vs a headless MesenCE. Original verification against SA-1's `SD F-1 Grand Prix` (159/239 rows changed, 232/237 checkable rows matching the predicted signature) and all 24 Super FX/GSU goldens still holds. `docs/ppu.md` §Mid-scanline/HDMA-driven register timing. |
