...
| Item | Disposition | Detail |
|---|---|---|
| 65816 `(dp,X)` emulation `DL!=0` high-byte wrap | **Remediated** (toward hardware) | The WDC 65816 silicon bug wrapping the `(dp,X)` pointer high byte within the page when `E=1 && DL!=0` is now modelled (`read_dp_x_ptr16`), matching bsnes/ares `readDirectX`, MesenCE `GetDirectAddressIndirectWordWithPageWrap`, and gilyon `cputest-full` (which now passes 1610/1610). SingleStepTests reads it linearly and is the outlier here (247 `(dp,X)` emu cases), so the fix costs SST agreement to gain hardware correctness — a documented inter-reference divergence resolved toward hardware. `docs/adr/0002`, `docs/cpu.md`. |
| 65816 `fc.e` (`JSR (addr,X)`, emulation-mode push) | **No-stricter-oracle-available** (hardware-correct) | `$FC`'s return-address push **escapes the emulation-mode page-1 stack bound** — on a page-boundary crossing (`S=$xx00`) the second byte lands at `$00FF`, not `$01FF`. Confirmed hardware-correct by **three independent SNES-accurate implementations**: bsnes/ares (one lineage — byuu's WDC65816 core) `instructionCallIndexedIndirect` (`pushN`, full-`S` decrement), snes9x `OpFCE1` (`PushW`/`WRAP_BANK`, with the source comment *"JSR (a,X) is a new instruction, and so doesn't respect the emu-mode stack bounds"*), and MesenCE `JSR_AbsIdxXInd` (`PushWord(PC, allowEmulationMode=false)` + `RestrictStackPointerValue()`); the AccuracySNES `A3.08` result is cross-validation against snes9x + Mesen2, not a separate oracle. SingleStepTests confines the push to page 1 (43 `fc.e` cases) and is the lone outlier — RustySNES matches these implementations on every physically-reachable state (`S.h=$01`), so no fix. `docs/adr/0002`, `docs/cpu.md`. |
...
| Mid-scanline/HDMA-driven register timing (the "Air Strike Patrol BG3 scroll" case) | **Remediated** (`v0.8.0`; deepened to dot resolution in `v1.21.0`) | Originally the PPU composited each line at a single `RENDER_DOT` (dot 276) so a per-line HDMA register write became visible starting the following line. As of `v1.21.0` the **per-dot compositor** (`docs/adr/0014`, T-CA-10) is the sole renderer: its compose/draw stage runs one column at a time against live registers, so a mid-line CGRAM/OAM access during active display and an `INIDISP` change reach only the not-yet-drawn columns (the BG/sprite line fetch is still line-wide; mid-line scroll fetch-ahead is Phase 4c, not yet landed) — validated 29/29 on undisbeliever vs a headless MesenCE. Original verification against SA-1's `SD F-1 Grand Prix` (159/239 rows changed, 232/237 checkable rows matching the predicted signature) and all 24 Super FX/GSU goldens still holds. `docs/ppu.md` §Mid-scanline/HDMA-driven register timing. |
| Open-bus-via-HDMA-latch (the "Speedy Gonzales stage 6-1" case) | **Remediated** | The naive prototype (update `open_bus` on all four A/B read *and* write methods) broke the 24 Super FX/GSU goldens; the root cause was found and the correct rule landed with the DMA-latch fix below — **DMA/HDMA *reads* update the latch, *writes* never do** (ares/bsnes `CPU::Channel`). The same `DmaBus for Bus` path (`read_a`/`read_b`) serves both GP-DMA and HDMA transfers, so the HDMA-latch case (an HDMA byte re-latching the bus between a CPU fetch and its execution — the Speedy Gonzales softlock-breaker) is covered by the same code. The 24 GSU goldens were re-blessed to the correct value (`BLESS_SUPERFX=1`); every other assertion in `superfx_oncart` is unaffected. `docs/scheduler.md` §Open bus via DMA/HDMA. |
...
| DRAM refresh (40 clocks/scanline) | **Remediated** | Implemented as a per-line 40-clock CPU stall at line-clock 536 (`bus.rs` `DRAM_REFRESH_*`). The prior "out-of-scope" rested on a frame-length measurement that is *refresh-insensitive* (the frame is PPU-rollover-fixed at 357,368 clocks for NTSC, so the stall reallocates CPU-vs-PPU progress without inflating the frame — the re-run probe confirms no drift). Validated: the H-IRQ raster boundary now varies with the ISR/refresh straddle exactly as MesenCE does, AccuracySNES `B3.01` detects the 10-dot pause (variant 2), and the shifted framebuffer regression goldens were re-blessed. `docs/dram-refresh.md`, `docs/scheduler.md` §DRAM refresh. |
