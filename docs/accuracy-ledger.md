...
| A5.04 (decimal costs no cycles) and A5.05 (16-bit RMW is +2) | **Deferred** | Both behaviours are well documented, but the H-counter measurement primitive is not yet precise enough to assert them: the comparison carries several dots of phase jitter (CPU cycles are 6/8 master clocks, a dot is 4) and long sequences wrap past the end of a scanline. Four cycle tests that *are* reliable ship as scored (A5.01/02/03/06). Loosening the tolerance until the other two passed would have defeated the point of measuring. |
