...
| DRAM refresh (40 clocks/scanline) | **Remediated** | Implemented as a per-line 40-clock CPU stall at line-clock 536 (`bus.rs` `DRAM_REFRESH_*`). The prior "out-of-scope" rested on a frame-length measurement that is *refresh-insensitive* (the frame is PPU-rollover-fixed at 357,368 clocks, so the stall reallocates CPU-vs-PPU progress without inflating the frame — the re-run probe confirms no drift). Validated: the H-IRQ raster boundary now varies with the ISR/refresh straddle exactly as MesenCE does, AccuracySNES `B3.01` detects the 10-dot pause (variant 2), and the shifted framebuffer regression goldens were re-blessed. `docs/dram-refresh.md`, `docs/scheduler.md` §DRAM refresh. |
