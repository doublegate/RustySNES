...
| 65816 `fc.e` (`JSR (addr,X)`, emulation-mode push) | **No-stricter-oracle-available** (hardware-correct) | `$FC` pushes the return address with `pushN` (full-16-bit `S`, escaping page 1 mid-instruction) per bsnes `instructionCallIndexedIndirect`, cross-validated by AccuracySNES `A3.08` on snes9x + Mesen2. SingleStepTests confines the push to page 1 (43 `fc.e` cases) and is the outlier — RustySNES is already hardware-correct, no fix needed. `docs/adr/0002`, `docs/cpu.md`. |
