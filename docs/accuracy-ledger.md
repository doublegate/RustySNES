...
| 65816 `fc.e` (`JSR (addr,X)`, emulation-mode push) | **No-stricter-oracle-available** (hardware-correct) | `$FC`'s return-address push **escapes the emulation-mode page-1 stack bound** — on a page-boundary crossing (`S=$xx00`) the second byte lands at `$00FF`, not `$01FF`. Confirmed hardware-correct by **all four** SNES-accurate references: bsnes/ares `instructionCallIndexedIndirect` (`pushN`, full-`S` decrement), snes9x `OpFCE1` (`PushW`/`WRAP_BANK`, with the source comment *"JSR (a,X) is a new instruction, and so doesn't respect the emu-mode stack bounds"*), and MesenCE `JSR_AbsIdxXInd` (`PushWord(PC, allowEmulationMode=false)` + `RestrictStackPointerValue()`); also cross-validated by AccuracySNES `A3.08` on snes9x + Mesen2. SingleStepTests confines the push to page 1 (43 `fc.e` cases) and is the lone outlier — RustySNES matches the four references on every physically-reachable state (`S.h=$01`), so no fix. `docs/adr/0002`, `docs/cpu.md`. |
