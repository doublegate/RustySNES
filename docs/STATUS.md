...
**Current release:** `v0.8.0 "Community"` (`v0.1.0 "Foundation"`, `v0.2.0 "Persistence"`,
`v0.3.0 "Continuum"`, `v0.4.0 "Completion"`, `v0.5.0 "Fidelity"`, `v0.6.0 "Shippable"`, and
`v0.7.0 "Resolution"` precede it; see `to-dos/VERSION-PLAN.md` for the full breadth-inclusive
`v0.7.0`→`v1.0.0` ladder).
...
`v0.8.0` lands Phase 8 in full (the debugger overlay + 65C816 read/write watchpoints +
`rustysnes_cpu::disasm`, plus a follow-up session's disassembly view/PC breakpoints/step
controls (T-81-001 PR B) and a new non-intrusive `Bus::peek` read, Lua scripting/TAS, cheat
codes, the real wasm frontend, GGPO-style
rollback netplay, RetroAchievements — `to-dos/VERSION-PLAN.md`'s ladder has the per-item detail)
alongside the mid-scanline/HDMA-driven register timing fix, a systemic cart-layer open-bus fix, a
continued SPC7110 investigation that reclassified its boot gap as a ROM-identity issue rather
than an open bug (`docs/audit/spc7110-boot-crash-2026-07-08.md`), and Phase 7's niche
peripherals (Mouse/Super Scope/Super Multitap — `rustysnes_core::controller`, a real
serial-shift-register protocol ported from ares, not a stub). The save-state `FORMAT_VERSION`
bumped `2`→`3` for the peripherals' runtime state (`docs/adr/0006`'s bump log).
