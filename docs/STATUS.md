...
| `rustysnes-netplay` | rollback netplay | **T-82-002 — GGPO-style 2-player rollback netcode, ported from RustyNES's `RollbackSession` shape (scoped to 2 players — the SNES core has no multitap emulation). Bit-identical resimulation proven under both ideal and adverse (latency/jitter/10% loss) conditions (`tests/determinism.rs`); a real UDP transport (OS-level loopback tested) + a wasm32-clippy-verified WebRTC transport. Frontend wiring (Tools → Netplay…, its own drive loop independent of `emu-thread`) behind the `netplay` flag is native/UDP only — the browser SDP-negotiation UI is an honestly deferred, separate scope** |
