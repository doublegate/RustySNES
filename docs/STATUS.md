...
| AccuracySNES (first-party battery) | ✅ **80 / 80 scoring, 100.00%** | Group A (65816 CPU, 46 tests) + Group C (PPU, 30 tests) + Group B (5A22 access speed, RDNMI, multiply/divide — 9 tests) — 85 total, 80 scoring + 5 golden vectors. Cross-validated headlessly against **Mesen2 and snes9x** (both agree, 0 failures). Provenance gate green. The renderer-dependent PPU sub-groups, the rest of Group B, and Groups D-G land in later phases (`docs/accuracysnes-research-dossier.md` §5) |
