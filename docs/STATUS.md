...
| AccuracySNES (first-party battery) | ✅ **66 / 66 scoring, 100.00%** | Group A (65816 CPU, 43 tests) + Group C (PPU port mechanics, open bus, version detection, Mode 7 multiply, sprite over-flags — 26 tests) — 69 total, 66 scoring + 3 golden vectors. Cross-validated headlessly against **Mesen2 and snes9x** (both agree, 0 failures). Provenance gate green. The renderer-dependent PPU sub-groups and Groups B, D-G land in later phases (`docs/accuracysnes-research-dossier.md` §5) |
