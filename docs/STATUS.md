...
`v1.3.0 "Palimpsest"`, `v1.4.0 "Convergence"`, and the RustyNES-parity ladder
`v1.5.0 "Bedrock"` through `v1.20.0 "Aperture"` precede it; see `to-dos/VERSION-PLAN.md` for the
full ladder). **`v1.5.0`-`v1.20.0`** were frontend/tooling/CI/mobile work — a CI safety net
(`v1.5.0 "Bedrock"`), a MkDocs docs site + PWA + accuracy ledger (`v1.6.0 "Lighthouse"`), the
debugger extracted into its own module plus hex Memory / Memory Compare / Docs panels
(`v1.7.0 "Telemetry"`, `v1.7.1`, `v1.8.0 "Tracepoint"`), `rustysnes-script`'s `emu.read` widened to
the full 24-bit bus (`v1.9.0 "Marionette"`), HD-pack/RetroAchievements/shader/accessibility depth
(`v1.10.0 "Atelier"`-`v1.13.0 "Vantage"`), the Android/iOS mobile ladder
(`v1.14.0 "Foundry"`-`v1.18.0 "Dormant"`), a PGO/BOLT pipeline (`v1.19.0 "Afterburner"`), and a
UI/UX-parity pass (`v1.20.0 "Aperture"`) — all with **zero change** to the accuracy dashboard,
per-suite pass counts, or coprocessor tier matrix below, which stayed byte-identical throughout.
**`v1.21.0 "Touchstone"` is the accuracy release that finally moves them:** the per-dot PPU
compositor (`docs/adr/0014`, T-CA-10) becomes the sole renderer — its compose/draw stage runs
...
results + per-test B-skip + a Select WRAM debug viewer) with its battery at **354 of 443** dossier
assertions (300 on-cart + 54 rendered scenes).
...
| Super FX / GSU-1/2 | Argonaut RISC | **Core/Curated** | — (cart ROM) | **implemented** — full GSU core (`coproc::gsu`: complete Argonaut RISC instruction set + ALT-mode machine, the multiplier, ROM/RAM buffers, opcode cache, the branch-delay pipeline, and the PLOT/RPIX pixel-plot pipeline) + `SuperFxBoard` (`coproc::superfx`: LoROM Super FX map, GSU register window, CPU↔GSU ROM/RAM arbitration). No chip dump — the GSU program is in cart ROM; host-synced on the Go flag as a master-clock-stepped cothread (`Gsu::tick` per `Board::coprocessor_tick`, ares' `SuperFX::main`; each instruction's clock cost paced out one master tick at a time so a `Go` burst interleaves with the CPU instead of draining atomically). Validated by `superfx_oncart` (58 Krom GSU ROMs: SuperFx detection + GSU-executed liveness + a FillPoly-into-RAM plot-pipeline assertion + deterministic golden) + the per-opcode `GSUTest` suite + engine unit tests. Honesty gate green (`ORACLE_COPROCESSORS` ∋ SuperFx) |
...
| DSP-2 / DSP-3 / DSP-4 | µPD77C25 | BestEffort | µPD77C25 (shared) | **implemented** — `coproc::necdsp_variant::NecDspVariantBoard` reuses the DSP-1 µPD7725 LLE engine, title-detected (`Variant::detect`). DSP-2 uses the generic bit-0 DR/SR split; DSP-3 (`v1.24.0`) shares that split over the full `$8000–FFFF` window (SD Gundam GX, detected by raw Shift-JIS title bytes since its UTF-8 title is empty); DSP-4 needed a DSP-1-style half-boundary split instead (found + fixed against a real Top Gear 3000 boot-time 16-bit hardware check). Validated against real Dungeon Master (DSP-2), SD Gundam GX (DSP-3), and Top Gear 3000 (DSP-4) — real title + chip liveness |
| ST010 / ST011 | µPD96050 | BestEffort | µPD96050 (shared) | **implemented** — same `NecDspVariantBoard`, µPD96050 DR/SR bit-0 split + the DP battery data-RAM window. ST011 (`v1.24.0`) is the identical board at **15 MHz** (vs ST010's 11 MHz), routed from the `$F` chipset nibble. Validated against real F1 ROC II (ST010) and 2-dan Morita Shougi (ST011) — real title + chip liveness |
...
| ST018 | ARMv3 | BestEffort | — (separate ARM LLE) | **implemented (`v0.4.0`)** — `coproc::armv3`: a full ARMv3 (ARM6-class, pre-Thumb) CPU core (barrel shifter/condition-codes/ALU, mode-banked register file, 3-stage pipeline, the complete instruction set) + `St018Board` (a single combined `0x28000`-byte firmware dump, the `$3800`/`$3802`/`$3804` handshake registers, 16 KiB work RAM), driven by `Board::coprocessor_tick` rather than the SA-1 second-CPU hooks since this ARM core is entirely self-contained in `rustysnes-cart`. Detected via a title match (`NIDAN MORITASHOGI2`) on the confirmed real cart, Hayazashi Nidan Morita Shogi 2 — an earlier investigation wrongly assumed Star Ocean (which uses S-DD1 only). **`v1.24.0` follow-up: validated against a real Nidan Morita Shogi 2 dump** — detection + boot + determinism (`srtc_st018_oncart`); the shogi AI itself is gameplay-gated (runs only on the computer's move, unreachable in a headless boot), so it is not exercised, hence not golden-framebuffer validated |
| S-RTC | Sharp S-RTC | BestEffort | — (frozen time) | **implemented (`v0.4.0`)** — `coproc::sharprtc::SharpRtcBoard`: a standalone Sharp S-RTC (Daikaijuu Monogatari II, ExHiROM), a DIFFERENT chip/protocol from the Epson RTC-4513 paired with SPC7110 above — a 2-register (`$2800`/`$2801`) handshake over a 13-slot decimal clock file (`Ready -> Command -> Read`/`Write` state machine), seeded to a fixed epoch, never wall-clock-advanced. Wraps a base `ExHiRom` board. **`v1.24.0` follow-up: validated against a real Daikaijuu Monogatari II dump** — detection + boot + determinism (`srtc_st018_oncart`). The detection was **structurally broken** and is now fixed: the real cart declares chipset `$55` (nibble `$5`, not the `$F` custom family), so the old `$F`-arm title check was unreachable, and its title is the no-space `JYU`-romanized `DAIKAIJYUMONOGATARI2`, not the guessed `DAIKAIJUU MONOGATARI` — it is now title-detected up front regardless of nibble. The RTC handshake is usage-gated (read at specific moments), so not exercised in a headless boot; not golden-framebuffer validated |
