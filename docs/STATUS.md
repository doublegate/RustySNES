...
| AccuracySNES (first-party battery) | ✅ **55 / 55 scoring, 100.00%** | Group A (65816 CPU, 43 tests) + Group C sub-groups C1-C3 (PPU OAM/VRAM/CGRAM port mechanics, 13 tests) — 56 total, 55 scoring + 1 golden vector. Cross-validated headlessly against **Mesen2 and snes9x** (both agree, 0 failures). Provenance gate green. The renderer-dependent PPU sub-groups and Groups B, D-G land in later phases (`docs/accuracysnes-research-dossier.md` §5) |
