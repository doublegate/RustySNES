...
crash — see the coprocessor matrix below for the precise remaining failure; ST018 is not started.
...
| S-RTC | Sharp RTC-4513 | BestEffort | — (frozen time) | **implemented (`v0.4.0`)** — `coproc::sharprtc::SharpRtcBoard`: a standalone Sharp RTC-4513 (Daikaijuu Monogatari II, ExHiROM), a DIFFERENT chip/protocol from the Epson RTC-4513 paired with SPC7110 above — a 2-register (`$2800`/`$2801`) handshake over a 13-slot decimal clock file (`Ready -> Command -> Read`/`Write` state machine), seeded to a fixed epoch, never wall-clock-advanced. Wraps a base `ExHiRom` board. No commercial dump in the local corpus — unit-test-level coverage only, not golden-framebuffer validated; header detection is a best-effort title match, matching the existing CX4/SPC7110 posture |
