...
| AccuracySNES (first-party battery) | ✅ **70 / 70 scoring, 100.00%** | Group A (65816 CPU, 43 tests) + Group C (PPU port mechanics, access windows, open bus, version detection, Mode 7 multiply, sprite over-flags, frame geometry — 30 tests) — 73 total, 70 scoring + 3 golden vectors. Cross-validated headlessly against **Mesen2 and snes9x** (both agree, 0 failures). Provenance gate green. The renderer-dependent PPU sub-groups and Groups B, D-G land in later phases (`docs/accuracysnes-research-dossier.md` §5) |
