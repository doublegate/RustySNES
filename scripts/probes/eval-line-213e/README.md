...
- fires an **H-IRQ every scanline** at HTIME=300 (deep HBlank, after that line's sprite evaluation),
...
|---|---|
| **MesenCE (oracle)** | **100** |
| RustySNES per-dot | 101 (one line late) |
| RustySNES batch (shipped) | 102 (two late) |
...
the paint's `scan_y = self.v-1`) and set `range_over` at the dot the 33rd in-range sprite is found.
After that lands, the per-dot build should read **100** here.
