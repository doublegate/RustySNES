...
pub fn asm(tests: &[Test]) -> String {
    // The runtime's user-skip bitmap `V_USER_SKIP` is 64 bytes (`USER_SKIP_BYTES` in runtime.inc) at
    // $7E:E100, one bit per test, ending right before the measurement channel at $7E:E200. Fail the
    // build if the battery outgrows that capacity, rather than let `user_skip_check` / `toggle_skip`
    // write out of bounds into the measurement WRAM.
    assert!(
        tests.len() <= 64 * 8,
        "the user-skip bitmap (V_USER_SKIP, 64 bytes) holds {} tests; got {}",
        64 * 8,
        tests.len()
    );
