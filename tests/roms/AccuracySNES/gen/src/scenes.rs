...
pub const SCENES: &[Scene] = &[
    ...
    Scene {
        ...
        dossier: "C5.05",
        what: "Mode 4 with BG1 and BG2 both displayed. The mode's shape is unlike its neighbours: \
               BG1 is 8bpp and BG2 is 2bpp — three bits of colour depth apart, from the same \
               tilemap — and BG3 exists only as the offset-per-tile table, which is left inert \
               here so this scene is about the mode's layers rather than about OPT. A core that \
               reuses mode 3's depths, or mode 2's layer set, renders both layers at the wrong \
               depth and the difference is not subtle.",
        setup: &[
            "sep #$20",
            "lda #$04",
            "sta $2105         ; BGMODE 4",
    ...
    Scene {
        ...
        dossier: "C7.11",
        what: "Two sprites with the same tile number, one with the name-select attribute bit set. \
               A sprite's character address is `(Base << 13) + (tile << 4)`, plus \
               `(NameSelect + 1) << 12` when that bit is set — so the second sprite reads from a \
               part of VRAM the font never reached and draws nothing. A core that ignores the bit \
               draws two identical glyphs, which is the picture this scene exists to be different \
               from. Blank is a legible answer here only because the other sprite is not.",
        setup: &[
            "sep #$20",
            "stz $2105         ; BGMODE 0",
            "jsr scene_oam_reset",
    ...
    Scene {
        ...
        dossier: "C5.14",
        what: "A custom 4bpp tile whose four bitplanes each carry a distinct horizontal stripe: \
               plane 0 on rows 0-1 (colour 1), plane 1 on rows 2-3 (colour 2), plane 2 on rows 4-5 \
               (colour 4), plane 3 on rows 6-7 (colour 8). A 4bpp tile is two 2bpp halves -- planes \
               0/1 in the first 16 bytes, planes 2/3 in the next 16 -- so it reads out as four \
               coloured bands only if the decoder pairs the right byte offsets to the right planes. \
               Font tiles cannot test this: they carry no data in planes 2/3, so a core that swaps \
               the high half renders them identically. Here a wrong plane pairing recolours the \
               lower two bands. The tile fills the whole screen; the palette gives colours 1/2/4/8 \
               four distinct hues.",
        setup: &[
            "sep #$20",
            "lda #$01",
