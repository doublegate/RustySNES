...
; provenance: Documented (fullsnes and anomie's DSP doc: KON/KOFF are sampled every second output sample, so a KOFF pulse shorter than the poll interval is never seen; E7.08 is the counterpart showing a single KOFF write does release)
.proc test_e8_07
    .a16
    .i16
    rep #$30
...
@pass:
    ...
    jml test_restore
@fail1:
    ; the envelope left full scale after KOFF was set to $FF and cleared to $00 a few cycles later. The pair collapses into a single poll that reads $00, so nothing should have been released — a core acting on the write itself releases on the $FF and cannot take it back
    sep #$20
    .a8
    lda #$02
    sta f:$7EE010
