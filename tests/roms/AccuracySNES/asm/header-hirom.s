; AccuracySNES — HiROM cartridge header and interrupt vectors.
;
; The HiROM sibling of `header.s`. It differs only in the bytes that make the image HiROM with
; battery SRAM ($FFD5 map mode, $FFD6 chipset, $FFD7 ROM size, $FFD8 RAM size); everything else —
; the title, the vector table, the checksum-placeholder convention — is identical, so the emulator's
; header detector (`crates/rustysnes-cart/src/header.rs`) selects the HiROM board and its SRAM window.
;
; The checksum/complement words are placeholders; `accuracysnes-gen` patches them after linking.

.p816

.import reset
.import irq_stub
.import irq_trampoline
.import nmi_trampoline
.import brk_trampoline
.import cop_trampoline_e
.import cop_trampoline

.segment "HEADER"
    ; --- extended header, $FFB0-$FFBF ---
    .byte "00"                  ; $FFB0  maker code
    .byte "ACSN"                ; $FFB2  game code
    .res   6, $00               ; $FFB6  reserved
    .byte $00                   ; $FFBC  expansion flash size
    .byte $00                   ; $FFBD  expansion RAM size
    .byte $00                   ; $FFBE  special version
    .byte $00                   ; $FFBF  chipset subtype

    ; --- core header, $FFC0-$FFDF ---
title:
    .byte "ACCURACYSNES-HI"     ; $FFC0  title, space-padded to exactly 21 bytes
    .assert * - title <= 21, error, "cartridge title exceeds 21 bytes"
    .res   title + 21 - *, $20

    .byte $21                   ; $FFD5  map mode: HiROM, SlowROM
    .byte $02                   ; $FFD6  cartridge type: ROM + RAM + battery (has SRAM)
    .byte $06                   ; $FFD7  ROM size: log2(65536) - 10 = 6  (64 KiB)
    .byte $03                   ; $FFD8  RAM size: 1<<3 = 8 KiB SRAM
    .byte $01                   ; $FFD9  country: NTSC / US
    .byte $33                   ; $FFDA  developer ID: use the $FFB0 maker code
    .byte $00                   ; $FFDB  ROM version
    .word $0000                 ; $FFDC  checksum complement (patched post-link)
    .word $0000                 ; $FFDE  checksum            (patched post-link)

.segment "VECTORS"
    ; --- native mode, $FFE0-$FFEF ---
    .word $0000, $0000          ; $FFE0, $FFE2  unused
    .addr cop_trampoline        ; $FFE4  COP
    .addr brk_trampoline        ; $FFE6  BRK
    .addr irq_stub              ; $FFE8  ABORT
    .addr nmi_trampoline        ; $FFEA  NMI (through RAM, see V_NMI_VEC)
    .word $0000                 ; $FFEC  unused (the CPU always boots in emulation mode)
    .addr irq_trampoline        ; $FFEE  IRQ (through RAM, see V_IRQ_VEC)

    ; --- emulation mode, $FFF0-$FFFF ---
    .word $0000, $0000          ; $FFF0, $FFF2  unused
    .addr cop_trampoline_e      ; $FFF4  COP (emulation)
    .word $0000                 ; $FFF6  unused (BRK shares the IRQ vector in emulation)
    .addr irq_stub              ; $FFF8  ABORT
    .addr nmi_trampoline        ; $FFFA  NMI (emulation; same RAM pointer as native)
    .addr reset                 ; $FFFC  RESET — the entry point
    .addr brk_trampoline        ; $FFFE  IRQ / BRK (emulation)
