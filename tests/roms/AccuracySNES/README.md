...
Phase A shipped **Group A (65816 CPU)**, 43 tests. Phase B adds **Group C** — the OAM, VRAM and CGRAM/counter *port mechanics*, the PPU1/PPU2 open-bus latches, the version nibbles, the Mode 7 hardware multiply, the sprite over-flags, the VRAM access window, and the overscan vblank boundary — 30 tests — and opens **Group B** (5A22 access speed, `RDNMI` mechanics, multiply/divide) with 9. 85 total: 80 scoring + 5 golden vectors, all passing on RustySNES, Mesen2, and snes9x.
