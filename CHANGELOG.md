...
- **The S-DSP blocker is now identified and written down.** - (~73 assertions — BRR, pitch,
  envelopes, key on/off, the mixer) are all read back through DSP registers, and the /
  path does not work in this harness yet: a program that writes a register and reads it back gets
  zero, and the DSP file inspected from the host is entirely zero. Identical on RustySNES and
  snes9x, so it is the cart'''s sequence at fault rather than the cores.

  Two tests written against it were **withdrawn rather than recorded as goldens**. '''s
  treatment does not transfer: there the claim was specific and the disagreement was the finding,
  whereas a golden for "DSP addressing" that nobody can interpret adds noise rather than evidence.
   records the three candidate causes to check first.
