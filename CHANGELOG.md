...
- **Frontend parity with RustyNES: A/V capture, virtual pad, databases, macros (T-FP-G1).**
  - **A/V capture** (`crate::av_record`) to raw `.y4m` + `.wav`, printing the `ffmpeg` mux command on
    ...
    corrected** — silently resampling would make a broken capture look fine.
  - **Virtual pad** (`crate::virtual_pad`) — an on-screen controller for touch and as a visual input
    ...
    direction while tapping a face button is the normal case.
  ...
  - **Input macros** (`crate::input_macros`) — deliberately not a TAS movie, since a movie's seed and
    ...
    at 512 frames because forgetting the stop hotkey is the real failure mode.
...
- **Frontend parity with RustyNES: debugger instrumentation (T-FP-C1).**
  - **`rustysnes_core::trace`** — an instruction trace, a control-flow event log, and a WRAM access
    ...
    the operand — a `JSR (a,X)` has no static destination. An interrupt is detected **before** the
    opcode is consulted, via a new `Cpu::interrupts_taken` counter written only on the NMI/IRQ path
    (never per instruction, and excluded from save states): a step that vectors fetches no opcode at
    all, so the previous instruction's byte is what is left lying around, and classifying from it
    would record an NMI arriving after a `JSR` as a second `Call` pointing at the NMI vector.
  - The heat map masks its address to 24 bits, matching the CPU bus's own wrap at `$FF:FFFF`, so a
    window walking off the top of memory cannot report low-RAM heat belonging to a different
    address; and the **freeze list** keys on a canonical WRAM address, so `$00:0042` and `$7E:0042`
    — one byte — cannot become two entries whose per-frame pokes fight, with the later one silently
    winning.
  ...
  - **OAM panel** — the 544 OAM bytes had been in the snapshot since the overlay existed with nothing
    ...
    hex. Off-screen rows are dimmed rather than hidden, and the marker is biased toward *not*
    marking: it uses the maximum sprite extent (64 px) and the 239-line overscan display height, not
    224 — the panel cannot see the live `$2133` bit, and a game running overscan puts real sprites on
    lines 224-238.
...
- **The per-dot PPU compositor's background fetch is now incremental (T-CA-10 Phase 4c,
  `docs/adr/0014`).** A fetch cursor (`pd_fetch_bg_to`) runs 22 columns ahead of the draw cursor,
  building each column's pixels from **live** BG-data registers over the backdrop — the tiled modes
  via `fetch_bg_column` (scroll, tilemap, char base, tile size, mosaic, BG3 offset-per-tile), Mode 7
  via `fetch_mode7_column` (the M7A-D matrix, M7X/Y centre, M7HOFS/VOFS scroll, flips, mosaic,
  M7SEL) — then the line's latched sprites (`pd_sprite`, resolved once — sprites do not change from a
  BG-data write). So a mid-line write to `BGnSC`/`BGnNBA`/`BGnHOFS`/`BGnVOFS`/`MOSAIC`/OPT or a
  Mode-7 matrix/centre/scroll register now takes effect only on columns the fetch has not yet
  reached — the raster-scroll effects (mid-frame parallax splits, Mode-7 per-line transforms) that
  the previous whole-line-at-line-start fetch rendered flat. `bg_mode` is read live per column, so a
  mid-line `BGMODE` flip across the Mode-7 boundary switches the fetch path correctly in both
