# Anomie's SNES Timing Doc (vendored verbatim)

**Source:** Anomie <anomie@users.sourceforge.net>, "Anomie's SNES Timing Doc",
$Revision: 1126 $ ($Date: 2007-04-21 $). A long-circulated community reference on
SNES event timing (H/V counter latch positions, IRQ/NMI timing, DMA/HDMA cycle
stealing, APU/DSP timing).

**Provenance / licensing:** third-party community documentation, retrieved 2026-07-22
and preserved here verbatim for reference. No explicit permissive licence accompanies
the original; treat as **reference only** — cite it, but do not present its text as
this project's own, and prefer re-expressing derived facts in our own words (the same
discipline applied to the WDC datasheet citation in this directory). Not a scoring
oracle: like every emulator-era timing compilation, agreement with it does not prove
