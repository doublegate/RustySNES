...
2. The save-state/core API (v0.2.0) is **stable** — the contract every post-1.0 Reach feature
   ...
   **Checked (v0.6.0-era):** `FORMAT_VERSION` (`rustysnes-core::scheduler`) has stayed at `1`
   since `v0.2.0` even though `v0.4.0` added two new coprocessor board types (ST018, S-RTC) —
   real evidence `docs/adr/0006`'s per-board-opaque-section design holds up in practice, not
   just on paper: a new coprocessor extends the format without needing a version bump. **Real
   gap found, not yet closed:** no committed old-format save-state fixture + regression test
   proves actual backward-compatibility — only same-version round-trip determinism is tested
   ...
   `FORMAT_VERSION=1` fixture blob + a loadability regression test (the same Golden-Vector
