...
| **T-04-I** | The 256-opcode cycle sweep (`A5.01`–`A5.08`) | 1 mechanism | **an external per-opcode timing table** — measured, not assumed: `A5.08` shows the three references disagree with each other on instruction timing (RustySNES `101`, snes9x `100`), so no sweep can score against them. Also needs a safe-operand table + sandbox. `STP` excluded — it halts until reset |
