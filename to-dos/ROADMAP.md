...
| **T-FP-G** (G1 landed `v1.25.0`; TAStudio open) | The largest, most self-contained items, last: `av_record.rs`, `tastudio/` + `tastudio_panel.rs` + `movie_ui.rs` + `input_macros.rs`, `virtual_pad.rs` + `wasm_touch.rs`, `game_db.rs` + `genie_db.rs` | ~6 | nothing |
