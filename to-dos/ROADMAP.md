...
| **T-04-H** | The renderer-dependent rest of Group C (`C5`, `C6`, `C8`, `C10`, `C12`, most of `C9`, `C13.01`–`C13.06`) | ~42 | ~~a framebuffer oracle~~ **UNBLOCKED — MECHANISM LANDED.** `docs/adr/0013` accepted; cart scene loop + three hosts (in-repo, snes9x `--scenes`, Mesen2 `mesen_scenes.lua`) hashing a canonical 256x224 region against `tests/golden/accuracysnes-scenes.tsv`, gated by `crossval.sh`. **3 scenes blessed** (`C5.02`, `C8.10`, `C10.01`) — and all three found real RustySNES bugs (BG vertical fetch a line late; mosaic anchored to the BG instead of the screen). Remaining: ~39 more scenes, one per assertion |
