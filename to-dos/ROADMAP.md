...
| **T-04-D** | Group D — DMA / HDMA. **Started:** 11 tests — GP-DMA (`D1.01`, `D1.02`, `D1.05`-`D1.07`, `D1.09`/`D1.15`, `D1.10`) and the first HDMA pair (`D2.03`, `D2.04`). Found two defects: the unmodelled `$43xB` scratch latch and a WRAM->`$2180` transfer that wrote when hardware does not. Left: `D1.03`/`D1.04`/`D1.13`/`D1.14`, the rest of `D2`, and `D1.08`/`D3` which need care | ~24 | **a research top-up**: the dossier's DMA/HDMA sub-agent never returned, so `D1`/`D2` are under-sourced |
