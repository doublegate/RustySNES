...
| T-CA-13 | DRAM refresh per-line CPU stall | unmodeled (previously ledger "out-of-scope") | `bus.rs`, ares/MesenCE/fullsnes | **[x] Landed (2026-07-25).** 40-master-clock stall once per scanline at line-clock 536 (`DRAM_REFRESH_*`, nested `advance_master` on dot-133 completion). Reallocation, not addition — frame stays PPU-fixed at 357,368 clocks (re-run probe confirms no drift), correcting the old "would inflate the frame" conclusion (which measured a refresh-insensitive metric). Validates: H-IRQ raster boundary now varies with the ISR/refresh straddle vs MesenCE, `B3.01` detects the 10-dot pause (variant 2). Stateless → no save-format change. Shifted framebuffer regression goldens re-blessed. `docs/dram-refresh.md`. |
